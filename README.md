...
- **逐物件的個別 SQL 檔**（每個物件一檔，由 DacFx 官方引擎單獨產生，等同 VS 逐物件勾選匯出）
...
「切分SQL/」裡的每個檔都是**單一物件**的部署腳本，產生方式與 VS「結構描述比較」逐物件勾選匯出相同：對每個物件，只在 DacFx 結果中納入「該物件」再 `GenerateScript()`，由**官方引擎**決定該檔要包含什麼（該物件的 DDL、它的描述、相依模組刷新…），沒有「自己切 FullScript、再猜哪段屬於哪個物件」的風險。
...
> `FullScript.sql` 仍是依相依順序的權威單一部署腳本；逐物件檔則方便你像過去一樣一個物件一個物件地檢視與套用。
