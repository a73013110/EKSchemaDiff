...
public static class HtmlReportBuilder
{
    ...
    /// <summary>
    /// 把一行的行內片段渲染成 HTML：變更字詞包進 &lt;span class="seg"&gt; 以行內高亮，未變更字詞原樣輸出。
    /// 片段為空時退回整行純文字（保險）。
    /// </summary>
    private static string Cell(IReadOnlyList<DiffSegment> segments, string fallback)
    {
        if (segments is null || segments.Count == 0) return Enc(fallback);
        var sb = new StringBuilder();
        foreach (var seg in segments)
        {
            var enc = Enc(seg.Text);
            sb.Append(seg.Kind == SegmentKind.Changed ? $"<span class=\"seg\">{enc}</span>" : enc);
        }
        return sb.ToString();
    }
        ...
        bool ignoreWhitespace, DateTime generatedAt)
    {
        ...
        foreach (var row in rows)
        {
            ...
            rowHtml.Append($"<tr class=\"{className}\"><td class=\"num\">{leftNumber}</td><td class=\"code\"><pre>{Cell(row.LeftSegments, row.Left)}</pre></td><td class=\"mark\">{marker}</td><td class=\"num\">{rightNumber}</td><td class=\"code\"><pre>{Cell(row.RightSegments, row.Right)}</pre></td></tr>\r\n");
        }
