...
public sealed class SchemaComparer
{
        ...
        Func<string, string>? passwordPrompt = null)
    {
        ...
        profile.CompareOptions.ApplyTo(comparison.Options);
        profile.CompareOptions.ResolveExcludedObjectTypes(unrecognized);
...
public sealed class CompareSession
{
    ...
    public void ApplyInclusion(ISet<ObjectDifference> include)
    {
        foreach (var d in _differences)
        {
            if (include.Contains(d))
            {
                if (!d.Inner.Included) _result.Include(d.Inner);
            }
            else
            {
                if (d.Inner.Included) _result.Exclude(d.Inner);
            }
        }
    }
